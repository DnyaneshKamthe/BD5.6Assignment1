...
app.post('/tickets/update/:id', async (req, res) => {
  try {
    ...
    if (!ticketDetails) {
      return res
        .status(400)
        .json({ message: 'No such ticket found with this id' });
    }
...
app.get('/ticketCustomer/:id', async (req, res) => {
  try {
    ...
    if (results.length === 0) {
      return res
        .status(404)
        .json({ message: 'No customers found for this ticket ID' });
    }
...
app.post('/tickets/delete', async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: `Ticket with ID ${ticketId} deleted successfully.` });
  } catch (error) {
